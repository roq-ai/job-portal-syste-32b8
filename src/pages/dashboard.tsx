...
import { useRoqPlatformUserProfiles, useJobSeekerCount, useRecruiterCount, useApplicationCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const jobSeekerStat = useJobSeekerCount();

  const recruiterStat = useRecruiterCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Job Seekers',
      result: jobSeekerStat?.data,
      isLoading: jobSeekerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Recruiters',
      result: recruiterStat?.data,
      isLoading: recruiterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Applications',
