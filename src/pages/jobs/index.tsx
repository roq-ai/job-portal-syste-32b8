...
export function JobListPage(props: JobListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['recruiter', 'application.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'description.contains', 'location.contains', 'job_type.contains'],
          ...params.filters,
        },
        'job',
      ),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'job_type', header: 'Job Type', accessorKey: 'job_type' },
    {
      id: 'posted_date',
      header: 'Posted Date',
      accessorKey: 'posted_date',
      cell: ({ row: { original: record } }: any) =>
        record?.posted_date ? format(record?.posted_date, 'dd-MM-yyyy') : '',
    },
    hasAccess('recruiter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/recruiters/view/${record.recruiter?.id}`}>
              {record.recruiter?.job_posted}
            </Link>
          ),
        }
      : null,
